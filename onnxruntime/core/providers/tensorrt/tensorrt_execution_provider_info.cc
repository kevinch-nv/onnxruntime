...
constexpr const char* kDLACapability = "trt_dla_capability";
...
TensorrtExecutionProviderInfo TensorrtExecutionProviderInfo::FromProviderOptions(const ProviderOptions& options) {
  ...
  ORT_THROW_IF_ERROR(
      ProviderOptionsParser{}
          ...
          .AddAssignmentToReference(tensorrt::provider_option_names::kDLACapability, info.dla_capability)
...
ProviderOptions TensorrtExecutionProviderInfo::ToProviderOptions(const TensorrtExecutionProviderInfo& info) {
  const ProviderOptions options{
      ...
      {tensorrt::provider_option_names::kDLACapability, MakeStringWithClassicLocale(info.dla_capability)},
  };
  return options;
}
...
ProviderOptions TensorrtExecutionProviderInfo::ToProviderOptions(const OrtTensorRTProviderOptionsV2& info) {
  ...
  const ProviderOptions options{
      ...
      {tensorrt::provider_option_names::kDLACapability, MakeStringWithClassicLocale(info.trt_dla_capability)},
  };
  return options;
}
...
void TensorrtExecutionProviderInfo::UpdateProviderOptions(void* provider_options, const ProviderOptions& options, bool string_copy) {
  ...
  trt_provider_options_v2.trt_dla_capability = internal_options.dla_capability;
}
}  // namespace onnxruntime
